...
enum TransactionService {
	...
	SERVICE_EARN_BETTING = 0xACA75AAE,
	...
	SERVICE_EARN_PROPERTY_SALES = 0xB77DD8B4,
	...
	SERVICE_EARN_REFUND_ORBITAL_AUTO = 0x0CD9EBA6,
...
struct ScriptHeader {
	...
	int nativeCount;					//0x2C NEEDED
	__int64* localOffset;				//0x30
	char padding5[8];					//0x38
	__int64* nativeOffset;				//0x40 NEEDED
...
struct ScriptTable {
	...
	char padding[16];
	int count;
	ScriptTableItem* FindScript(Hash hash) {
		if (TablePtr == nullptr) {
			return nullptr;//table initialisation hasnt happened yet
		}
...
public:
	...
	char _0x03C8[0x10];
	__int64 ARGEdx; //0x03D8  

};

class CPlayerAngles {
public:
	char pad_0x0000[0x44];
	Vector2 ViewAngles; //0x44
};

struct CNetworkEventContext;
struct CNetworkEvent;
typedef bool(*tHandlers)(uint64_t NetEventContext, CNetGamePlayer* sender, uint64_t a3);
typedef void(*tSetupHandlers)(uint64_t NetEventContext, uint64_t a2, CNetGamePlayer* sender, uint64_t a4);
struct CNetworkEvent {
	char _0x0000[0x8]; //0x0
	char* (*GetEventName)(); //0x8
	char _0x0010[0x20]; //0x10
	//__int64(*Handler)(CNetworkEventContext*, DWORD64 r4, CNetGamePlayer* pSender, DWORD64 r6); //0x30
	//__int64(*Handler)(CNetworkEventContext*, uint64_t, CNetGamePlayer*, uint64_t); //0x30
	tSetupHandlers SetupHandler;
	tHandlers EventHandler; //HandleIncrementStatEventHook 0x38
	char _0x0038[0x20]; //0x38
};

struct CNetworkEventContext {
	CNetworkEvent* pVFTable; //0x00
	BYTE EventID; //0x8 //
	//char _0x000A[0x26];
	//DWORD Something;
	//DWORD Something2;
	//CNetGamePlayer* NetGamePlayer;
	//uint64_t* ScriptEventArgs; //temp
};